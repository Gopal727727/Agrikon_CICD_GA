...
const WeatherToday = ({ city }) => {
  ...
  const UNITS = 'metric';
  const API_URL = `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${UNITS}&appid=${API_KEY}`;
  ...
  const precipitation = weather.rain ? weather.rain["1h"] : 0; // Rain in the last hour
  const precipitationChance = precipitation > 0 ? 'High' : 'Low'; // Estimate precipitation chance based on rain data
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', textAlign: 'center', padding: '20px', backgroundColor: '#f0f8ff' }}>
      ...
      >
        ...
        >
          <h5>{new Date(weather.dt * 1000).toLocaleDateString()}</h5>
          <img
